...
DEV_SUBSTITUTIONS = {
    ...
    # Matras
    # normalize composed vs decomposed (Unicode normalization handles this)
    "े": "े",
    # Numerals
    "७": "७",   # normalize
}
...
def postprocess(raw_xml_output):
    ...
    cn_valid, cn_dev_fixed, cn_eng_fixed = validate_citizenship_number(
        cn_dev, cn_eng)
    nested["citizenship_number"]["dev"] = cn_dev_fixed
    nested["citizenship_number"]["eng"] = cn_eng_fixed
    if not cn_valid:
        warnings.append(
            f"citizenship_number format invalid: dev='{cn_dev}' eng='{cn_eng}'")
    confidence["citizenship_number.dev"] = field_confidence(
        "citizenship_number.dev", cn_dev_fixed, cn_valid)
    confidence["citizenship_number.eng"] = field_confidence(
        "citizenship_number.eng", cn_eng_fixed, cn_valid)

    # Step 3: Validate BS date
    bs = nested["dob"]["bs"]
    dob_valid = validate_bs_date(
        bs.get("year", ""), bs.get("month", ""), bs.get("day", ""))
    if not dob_valid:
        warnings.append(f"dob.bs invalid: {bs}")
    confidence["dob.bs"] = field_confidence(
        "dob.bs", bs.get("year", ""), dob_valid)
    ...
    confidence["gender.dev"] = field_confidence(
        "gender.dev", gen_dev_fixed, gen_valid)
    ...
    confidence["issue_date.bs"] = field_confidence(
        "issue_date.bs", issue_fixed, issue_valid)
    ...
    confidence["citizenship_type"] = field_confidence(
        "citizenship_type", ctype, ctype_valid)

    # Step 7: Simple confidence for name fields
    name_dev = nested["name"]["dev"]
    confidence["name.dev"] = field_confidence(
        "name.dev", name_dev, bool(name_dev))
